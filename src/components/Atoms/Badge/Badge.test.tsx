...
describe('Badge 컴포넌트', () => {
  ...
  describe('count 포맷팅', () => {
    test('기본 maxCount(999) 이하의 숫자는 그대로 표시되어야 한다', () => {
      render(<Badge count={123} />);
      expect(screen.getByText('123')).toBeInTheDocument();
    });

    test('maxCount를 초과하면 "maxCount+" 형식으로 표시되어야 한다', () => {
      render(<Badge count={1234} />);
      expect(screen.getByText('999+')).toBeInTheDocument();
    });

    test('커스텀 maxCount를 초과하면 "maxCount+" 형식으로 표시되어야 한다', () => {
      render(<Badge count={150} maxCount={99} />);
      expect(screen.getByText('99+')).toBeInTheDocument();
    });

    test('maxCount와 같은 값은 그대로 표시되어야 한다', () => {
  ...
  describe('showZero prop', () => {
    test('showZero가 false일 때 0은 렌더링되지 않아야 한다 (기본값)', () => {
      const { container } = render(<Badge count={0} />);
      expect(container.firstChild).toBeNull();
    });

    test('showZero가 true일 때 0이 렌더링되어야 한다', () => {
  ...
  describe('유효하지 않은 count 처리', () => {
    test('음수는 렌더링되지 않아야 한다', () => {
      const { container } = render(<Badge count={-5} />);
      expect(container.firstChild).toBeNull();
    });

    test('NaN은 렌더링되지 않아야 한다', () => {
      const { container } = render(<Badge count={NaN} />);
      expect(container.firstChild).toBeNull();
    });

    test('Infinity는 렌더링되지 않아야 한다', () => {
  ...
  describe('접근성 (a11y)', () => {
    test('기본 aria-label이 "알림 {숫자}" 형식이어야 한다', () => {
      render(<Badge count={5} />);
      const badge = screen.getByRole('status');
      expect(badge).toHaveAttribute('aria-label', '알림 5');
    });

    test('maxCount 초과시 aria-label이 "알림 {maxCount}+" 형식이어야 한다', () => {
      render(<Badge count={150} maxCount={99} />);
      const badge = screen.getByRole('status');
      expect(badge).toHaveAttribute('aria-label', '알림 99+');
    });

    test('커스텀 aria-label을 전달할 수 있어야 한다', () => {
